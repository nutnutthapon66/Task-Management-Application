...
class HttpRequest {
  ...
  public get<T = unknown>(endPoint: string, params?: object, config?: AxiosRequestConfig): Promise<T> {
    return this.axiosInstance.get<T, T>(endPoint, { params, ...config })
  }

  public post<T = unknown>(endPoint: string, data?: object, config?: AxiosRequestConfig): Promise<T> {
    return this.axiosInstance.post<T, T>(endPoint, data, config)
  }

  public put<T = unknown>(endPoint: string, data?: object, config?: AxiosRequestConfig): Promise<T> {
    return this.axiosInstance.put<T, T>(endPoint, data, config)
  }

  public patch<T = unknown>(endPoint: string, data?: object, config?: AxiosRequestConfig): Promise<T> {
    return this.axiosInstance.patch<T, T>(endPoint, data, config)
  }

  public delete<T = unknown>(endPoint: string, params?: object, data?: object): Promise<T> {
